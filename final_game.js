...
function detectCollidearrowWithEnemy() {
    if (!arrowFire) return; // Only check collision if a arrow is fired

    let arrow = inputTriangles[1]; // Assuming inputTriangles[1] is the arrow
    let arrowPos = vec3.fromValues(arrow.center[0] + arrow.translation[0], arrow.center[1] + arrow.translation[1], 0); // arrow position in 3D

    for (let i = 2; i < inputTriangles.length; i=i+4) {
            let enemy = inputTriangles[i]; 
            

            let arrow1 = inputTriangles[i+1];// Iterate through all enemys
            let arrow2 = inputTriangles[i+2];
            let arrow3 = inputTriangles[i+3];

            if (enemy.attack === false) {
                let enemyPos = vec3.fromValues(
                    enemy.center[0] + enemy.translation[0],
                    enemy.center[1] + enemy.translation[1],
                    0
                );
    
                let distance = vec3.distance(arrowPos, enemyPos);
                let collisionThreshold = 0.15;

                // Check for intersection (bounding box collision detection)
                if ( distance< collisionThreshold) {
                    console.log(enemy.center);
...
function detectCollideEnemyToSpaceship(i, ship) {
    let enemy = inputTriangles[i];
    let arrow1 = inputTriangles[i+1];// Iterate through all enemys
    let arrow2 = inputTriangles[i+2];
    let arrow3 = inputTriangles[i+3];
    let shiparrow = inputTriangles[1];
    let enemyPos = vec3.fromValues(enemy.center[0] + enemy.translation[0], enemy.center[1] + enemy.translation[1], 0); // arrow position in 3D
    ...
    let distance = vec3.distance(enemyPos, shipPos);
...
function updateSinusoidalTranslation( ) {
    ...
    while(i<start+4) {
        ...
        detectCollideEnemyToSpaceship(i,inputTriangles[0]);
        if (!currSet.visible ) {
            i=i+1;
            continue;
        }
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "Space": 
            ...
            if (!arrowFire) { // Only fire if the arrow isn't already fired

                arrowFire = 1;

                // Reset arrow position to the spaceship's position before firing
...
function updatearrowTranslation() {
    boundaryHit = false;
...
function updateenemyarrowTranslation() {
    // boundaryHit = false;
...
function resetarrow() {
    arrowFire = 0; // Stop arrow firing
    let arrow = inputTriangles[1];
    let spaceship = inputTriangles[0];

    // Reset arrow position to spaceship
    arrow.translation[1] = spaceship.translation[1];
    arrow.translation[0] = spaceship.translation[0];

    clearInterval(translationIntervalId); // Stop the arrow movement interval
}
...
function main() {
    ...
    setupWebGL("galaxy.jpg"); // set up the webGL environment
    ...
    setInterval(detectCollidearrowWithEnemy,100);
    requestAnimationFrame(updateSinusoidalTranslation);
